...
# Configurações MQTT (Ajuste para o seu broker)
BROKER = "mosquitto-service"
TOPICO_SPLIT_SEND = "subscribe_topic_split_inference"
TOPICO_SPLIT_RECEIVE = "prediction"

def on_message(client, userdata, message):
    global recebeu_resultado, tempo_final
    if message.topic == TOPICO_SPLIT_RECEIVE:
...
mqtt_client = mqtt.Client(client_id="Client_Inference_Test")
mqtt_client.on_message = on_message
mqtt_client.connect(BROKER)
mqtt_client.subscribe(TOPICO_SPLIT_RECEIVE)
...
def split_inference(data, model):
    ...
    head_model = tf.keras.Sequential(model.layers[:2]) 
    ...
    mqtt_client.publish(TOPICO_SPLIT_SEND, payload)
